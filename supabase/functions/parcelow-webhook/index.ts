...
async function processWebhookEvent(event: ParcelowWebhookEvent, supabase: any) {
    const { data: parcelowOrder } = event
    ...
    if (newStatus === 'completed' && parcelowOrder.payments && parcelowOrder.payments.length > 0) {
        updateData.metadata = {
            ...
            total_brl: parcelowOrder.payments[0].total_brl, // Valor real com juros
    ...
    if (payment.program_id && newStatus === 'completed') {
        ...
        if (enrollError) {
            ...
            // 4. Gerar contrato (se disponível)
            try {
                ...
                console.log('[Parcelow Webhook] ✅ Enrollment contract triggered')
            } catch (pdfErr) {
                console.error('[Parcelow Webhook] ⚠️ Error triggering contract:', pdfErr.message)
            }
        }
    }
