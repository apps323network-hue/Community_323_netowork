      ...
      <div class="flex items-center justify-between h-16 sm:h-20">
        ...
        <div class="hidden md:block">
          <div class="ml-10 flex items-baseline space-x-6">
            ...
            <RouterLink
              to="/"
              class="relative px-3 py-2 text-sm font-medium text-gray-400 dark:text-gray-400 hover:text-primary dark:hover:text-secondary transition-colors group"
            >
              Comunidade
              <span class="absolute bottom-0 left-0 w-full h-0.5 bg-primary dark:bg-secondary transform scale-x-0 group-hover:scale-x-100 transition-transform"></span>
            </RouterLink>
            <RouterLink
              to="/eventos"
              class="relative px-3 py-2 text-sm font-medium transition-colors group"
              :class="route.path === '/eventos' 
                ? 'text-white dark:text-white' 
                : 'text-gray-400 dark:text-gray-400 hover:text-primary dark:hover:text-secondary'"
            >
              Eventos
...
async function handleLogout() {
  ...
  // Redirecionar imediatamente - o signOut pode acontecer em background
  window.location.href = '/login'
  
  // Tentar fazer signOut em background (não esperar)
  authStore.signOut().catch(() => {
