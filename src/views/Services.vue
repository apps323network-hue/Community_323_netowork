    ...
    <div class="space-y-8 w-full">
        ...
        <div class="relative z-10 flex flex-col gap-6 px-6 py-16 md:px-12 md:py-20 text-center md:text-left items-center md:items-start">
          <div class="flex flex-col gap-3 max-w-2xl">
            <div class="inline-flex items-center gap-2 self-center md:self-start px-3 py-1 rounded-full bg-secondary/10 border border-secondary/20 backdrop-blur-sm shadow-[0_0_15px_rgba(0,243,255,0.1)]">
              ...
              <span class="text-secondary text-xs font-bold uppercase tracking-wider">Exclusivo para Membros</span>
            </div>
            ...
            <p class="text-gray-300 text-base md:text-lg font-medium leading-relaxed max-w-lg mt-2">
          ...
          <div class="flex flex-wrap gap-4 justify-center md:justify-start mt-2">
            <button class="flex items-center justify-center rounded-lg h-12 px-6 bg-gradient-to-r from-primary to-secondary text-black text-base font-bold transition-all shadow-[0_0_20px_rgba(0,243,255,0.3)] hover:shadow-[0_0_40px_rgba(0,243,255,0.5)] hover:scale-105" @click="exploreServices">
              Explorar Serviços
            </button>
            <button class="flex items-center justify-center rounded-lg h-12 px-6 bg-white/5 hover:bg-white/10 border border-white/10 text-white text-base font-medium backdrop-blur-sm transition-colors" @click="howItWorks">
              Como funciona
            </button>
          </div>
        </div>
      ...
      <div class="flex gap-3 overflow-x-auto p-4 no-scrollbar">
        <button
          ...
          :key="filter.id"
          :class="[
            'flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-full px-5 transition-all outline-none',
      ...
      <div>
        ...
        <div v-else-if="filteredServices.length === 0" class="flex flex-col items-center justify-center py-20 bg-surface-dark/50 rounded-2xl border border-white/10 backdrop-blur-sm">
          <span class="material-symbols-outlined text-gray-500 text-[64px] mb-4">search_off</span>
          <p class="text-gray-400 font-medium">Nenhum serviço encontrado nesta categoria.</p>
        </div>
      ...
      <div class="flex flex-col gap-8 pt-8">
        <div class="flex items-center justify-between border-b border-white/10 pb-4">
          <h2 class="text-white text-2xl font-bold tracking-tight">O que nossos membros dizem</h2>
          <a class="text-secondary text-sm font-bold hover:underline hover:text-white transition-colors cursor-pointer">Ver todos</a>
        </div>
      ...
      <!-- CTA Section -->
      <div class="bg-[#0a040f] border border-white/10 rounded-2xl py-12 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-8">
        <div class="flex flex-col gap-2 text-center md:text-left">
          <h2 class="text-white text-2xl font-bold">Precisa de algo personalizado?</h2>
          <p class="text-gray-400">Nossa equipe de concierge pode ajudar você a encontrar exatamente o que precisa.</p>
        </div>
        <button class="flex items-center gap-2 rounded-lg bg-transparent border border-secondary text-secondary hover:bg-secondary hover:text-black px-6 py-3 font-bold transition-all duration-300 shadow-[0_0_15px_rgba(0,243,255,0.2)] hover:shadow-[0_0_25px_rgba(0,243,255,0.5)]" @click="contactSupport">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows="4"
            class="w-full rounded-lg border border-white/10 bg-surface-dark p-3 text-sm text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
