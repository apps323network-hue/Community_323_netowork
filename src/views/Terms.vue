...
onMounted(() => {
  ...
  if (showAcceptButton.value) {
    ...
    scrollHandler = () => {
      ...
      fetch('http://127.0.0.1:7243/ingest/90aaf856-d3a0-403e-b154-71a179af14ae',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Terms.vue:scrollListener',message:'Scroll event post-fix',data:{scrollTop,headerHeight,spacerHeight,mainTop,headerTop,contentTop,headerExists:!!headerEl,mainExists:!!mainEl,spacerExists:!!spacerEl},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'H4'})}).catch(()=>{});
    }
